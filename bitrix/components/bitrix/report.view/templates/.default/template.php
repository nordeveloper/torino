...
<div class="reports-result-list-wrap">
	...
	{
		$err = 0;
		$chartXValueTypes = array('boolean', 'date', 'datetime', 'float', 'integer', 'string');
		$chartTypes = array(
									...
									if (is_int($columnIndex) && $columnIndex >= 0)
									{
										...
										switch ($chartInfo['requestData']['columnTypes'][$columnIndex])
										{
											...
											case 'datetime':
												if (!empty($dataValue))
												{
													if (!CheckDateTime($dataValue, CSite::GetDateFormat('SHORT')))
													{
														$dataValue = ConvertTimeStamp(strtotime($dataValue), 'SHORT');
													}
