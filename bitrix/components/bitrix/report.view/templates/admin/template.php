			...
			<div class="adm-filter-content">
				<div class="adm-filter-content-table-wrap">
					...
					<table cellspacing="0" class="adm-filter-content-table" style="display: table;">
						...
						<? if (substr(call_user_func(array($arParams['REPORT_HELPER_CLASS'], 'getOwnerId')),0,5) === 'sale_'): ?>
						<tr>
									...
									<div class="adm-filter-box-sizing">
										<span class="adm-select-wrap">
											<select class="adm-select" id="sale-currency-filter" name="F_SALE_CURRENCY">
												<?php
												$arCurrencies = call_user_func(array($arParams['REPORT_HELPER_CLASS'], 'getCurrencies'));
												?>
						...
						<? if (call_user_func(array($arParams['REPORT_HELPER_CLASS'], 'getOwnerId')) === 'sale_SaleProduct'):
								...
								<div class="adm-filter-alignment">
									<div class="adm-filter-box-sizing">
										<span class="adm-select-wrap-multiple">
											...
											<?php
											$arPriceTypes = call_user_func(array($arParams['REPORT_HELPER_CLASS'], 'getPriceTypes'));
											?>
...
function getResultColumnDataType(&$viewColumnInfo, &$customColumnTypes = array())
{
	$dataType = null;
	...
	{
		$dataType = $viewColumnInfo['field']->getDataType();
	}
				...
				for ($i = 0; $i < $nGroupValues; $i++)
				{
					...
					if ($cur != $prev || $bLastValue)
					{
						...
						while ($n-- > 0)
						{
							...
							/*if (method_exists($arParams['REPORT_HELPER_CLASS'], 'formatResultGroupingValue'))
							{
								// format group value
								call_user_func(
									array($arParams['REPORT_HELPER_CLASS'], 'formatResultGroupingValue'),
									array(
										'k' => $groupValueKey,
										'v' => &$groupValue,
							...
							foreach ($arSubTotal as $k => $subValue)
							{
								...
								if ($bGroupingSubtotal || $level == $nGroups - 1)
								{
									...
									if (method_exists($arParams['REPORT_HELPER_CLASS'], 'formatResultGroupingTotal'))
									{
										...
										call_user_func(
											array($arParams['REPORT_HELPER_CLASS'], 'formatResultGroupingTotal'),
											array(
												'k' => $subValueKey,
												'v' => &$finalSubValue,
...
<?php
	...
	function prepareChartData(&$arResult, &$arGroupingResult = null)
	{
		...
		$xColumnDataType = getResultColumnDataType($arResult['viewColumns'][$xColumnIndex], $arResult['customColumnTypes']);
