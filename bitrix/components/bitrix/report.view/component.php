...
try
{
	...
	$arResult['fieldsTree'] = CReport::generateColumnTree($chains, $entity, $arParams['REPORT_HELPER_CLASS']);
	unset($chains);
	...
	{
		$options['RT_TIME_INTERVAL'] = "BETWEEN ".$db_date_from." AND ".$db_date_to;
	}
	else if (!is_null($date_from))
	{
		$options['RT_TIME_INTERVAL'] = "> ".$db_date_from;
	}
	else if (!is_null($date_to))
	{
		$options['RT_TIME_INTERVAL'] = "< ".$db_date_to;
	}
	else
	{
		$options['RT_TIME_INTERVAL'] = " IS NOT NULL";
	}
	// </editor-fold>
	...
	foreach ($settings['select'] as &$elem)
	{
		...
		CReport::appendHrefSelectElements($elem, $fList, $entity, $arParams['REPORT_HELPER_CLASS'], $select);
	}
	unset($elem);
	// </editor-fold>
	...
	foreach ($settings['select'] as $num => $elem)
	{
		...
		list($alias, $selElem) = CReport::prepareSelectViewElement($elem, $settings['select'], $is_init_entity_aggregated, $fList, $fChainList, $arParams['REPORT_HELPER_CLASS']);
		...
		elseif ($fType == 'string' && empty($elem['aggr']))
		{
			$defaultSort = 'ASC';
		}
		else
		...
		foreach ($fInfo as $k => &$fElem)
		{
			if (is_array($fElem) && $fElem['type'] == 'field' && (int) $fElem['changeable'] > 0)
			{
				if (preg_match('/__COLUMN__(\d+)/', $fElem['name'], $match))
				{
					...
					$data_type = $view['field']->getDataType();
				...
				if ($field instanceof Entity\ReferenceField)
				{
					...
					$value = $tmpElem['value'];
					$changeableFiltersEntities[$field->GetDataType()] = true;
				}
				else
				{
					$value = $fElem['value'];
		...
		foreach ($fInfo as $k => &$fElem)
		{
			if (is_array($fElem) && $fElem['type'] == 'field')
			{
				...
				if ($chain->hasBackReference())
				{
					...
					// add value filter
					$_sub_filter[CReport::$iBlockCompareVariations[$fElem['compare']].$fElem['name']] = $fElem['value'];
	...
	$fullHumanTitles = CReport::collectFullHumanTitles($arResult['fieldsTree']);
	...
	CReport::rewriteUserShortName($select, $arParams['USER_NAME_FORMAT'], $entity);
	...
	{
		$totalSelect['TOTAL_'.$k] = array(
			'data_type' => 'integer',
			'expression' => array('SUM(%s)', $k)
		);
	}
	...
	if ($need_concat_rows && $grcDataPrimaryValues)
	{
		...
		foreach ($grcSettingsNum as $num)
		{
			...
			CReport::appendHrefSelectElements($elem, $fList, $entity, $arParams['REPORT_HELPER_CLASS'], $grcSelect);
			...
			CReport::rewriteUserShortName($grcSelect, $arParams['USER_NAME_FORMAT'], $entity, true);
